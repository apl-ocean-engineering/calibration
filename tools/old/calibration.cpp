...
class CalOpts : public CalibrationOpts {
  public:
    CalOpts()
      : CalibrationOpts(),
      inFiles(),
      ignoreCache( false ),
      retryUnregistered( false )
  {;}
  ...
  protected:
    ...
    virtual void doParseCmdLine( TCLAP::CmdLine &cmd, int argc, char **argv )
    {
      ...
      ignoreCache = ignoreCacheArg.getValue();
      retryUnregistered = retryUnregArg.getValue();
      inFiles = inFilesArg.getValue();
