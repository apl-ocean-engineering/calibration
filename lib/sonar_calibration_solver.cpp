...
struct SonarCalibrationError {
  SonarCalibrationError( Vector2f vPoint, Vector3f sPoint )
      : _vPoint( vPoint ), _sPoint( sPoint )
  {;}
                  ...
                  T* residuals) const
  {
    ...
    //
    const T sp[3] = { T(_sPoint[0]), T(_sPoint[1]), T(_sPoint[2]) };
    T p[3];
    ...
    residuals[0] = predicted[0] - T( _vPoint[0] );
    residuals[1] = predicted[1] - T( _vPoint[1] );
    return true;
  }
...
struct SonarCalibrationErrorFactory {
  ...
  void add( ceres::Problem &problem, const Vector2f &vPoint, const Vector3f &sPoint )
  {
    ceres::CostFunction *costFunction = (new ceres::AutoDiffCostFunction<SonarCalibrationError, 2, 6>(
            new SonarCalibrationError( vPoint, sPoint ) ) );
...
bool SonarCalibrationSolver::solve( const SonarCalData &data, Result &result )
{
  ...
  for( size_t i = 0; i < data.size(); ++i ) {
        factory.add( problem, data[i].img, data[i].sonar );
  }
